...
This Quarto doc is the built-in example from the `gtsummary` R package. You can get the same file by running the code below:
...
Tables created with {gtsummary} can be integrated into R markdown documents. The {gtsummary} package was written to be a companion to the [{gt} package](https://gt.rstudio.com/) from RStudio, and {gtsummary} tables are printed using {gt} when possible. Currently, {gt} supports **HTML** output, with **LaTeX** and **RTF** planned for the future.
